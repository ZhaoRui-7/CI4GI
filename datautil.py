...
def compute_customized_interaction_graph(train_ui_u, train_ui_i, n_user,n_group, n_item, aug_dict=None):
    adj_row = train_ui_u
    adj_col = [i + n_user for i in train_ui_i]

    edge_index = torch.LongTensor([adj_row + adj_col, adj_col + adj_row])#两个list拼起来

    adj_sp, edge_weight = normalize_edge(edge_index, n_user + n_item)
    trend = edge_weight
    return trend, edge_index, adj_sp.to_dense().float()
