...
if __name__ == "__main__":
    ...
    parser.add_argument("--user_ssl_thershold", type=float, default=1.0)
    parser.add_argument("--user_side_ssl", type=int, default=1)
    parser.add_argument("--user_add_gitem_weight", type=int, default=1.0)
    ...
    parser.add_argument("--topK", type=list, default=[5, 10, 20,50])
    ...
    hg_dict = {
        ...
        "UserHGSSL":dataset.user_hg_for_ssl.to(device),
        ...
        "UIadj": dataset.adj.to(device),
        "GIadj": dataset.group_item_matrix_for_ssl.to(device),
        "UIHGADD":dataset.user_item_matrix_for_add.to(device)
    }
    ...
    for t in range(args.run_times):
        ...
        rec_model = model.CI4GI(num_user, num_item, num_group, args.emb_dim, args.layers, hg_dict,
                                             args.user_side_ssl, args.group_side_ssl,args.user_ssl_thershold,args.user_add_gitem_weight)
        rec_model.to(device)
        ...
        for epoch_id in range(args.epoch):
            ...
            for batch_id, (u, pos_i, neg_i) in enumerate(train_loader):
                ...
                bpr_loss, reg_loss, ssl_loss_1,ssl_loss_2, all_user_emb_i_side, all_user_emb_g_side = rec_model.bpr_loss(user_input, pos_item, neg_item)
                if args.user_side_ssl == 0 and args.group_side_ssl == 0:
                    loss = bpr_loss + args.reg_coef * reg_loss
                else:
                    weight = 1/(1+np.exp(-kl_loss_k*(epoch_id-kl_loss_s0)))
                    loss = bpr_loss + args.reg_coef * reg_loss + args.ssl_coef * weight* ssl_loss_1 + args.ssl_coef * (1-weight) * ssl_loss_2
                    ssl_losses += weight* ssl_loss_1 + (1-weight) * ssl_loss_2
                reg_losses += reg_loss
                bpr_losses += bpr_loss
